...
void AcGiVectorizer::updateLayerTraits(const OdGiSubEntityTraitsData& traits) const {
  OdGiDrawablePtr layer = OdDbObjectId(traits.layer()).openObject();
  if (layer_record == nullptr)
     return;

  effect_layer_data_.setTrueColor(layer_record->color().entityColor());
  effect_layer_data_.setLineweight(layer_record->lineWeight());
  effect_layer_data_.setLinetype(layer_record->linetypeObjectId());
  effect_layer_data_.setByBlock(OdDbSymUtil::isLayerZeroName(layer_record->getName()));
  effect_layer_data_.setFrozen(layer_record->isFrozen());
  effect_layer_data_.setOff(layer_record->isOff());
  effect_layer_data_.setLocked(layer_record->isLocked());
  effect_layer_data_.setPlottable(layer_record->isPlottable());
  effect_layer_data_.m_plotStyleNameId = layer_record->plotStyleNameId();
  effect_layer_data_.m_plotStyleNameType = OdDb::kPlotStyleNameById;
}
